...
export function BotasHombre() {
    ...
    return (
        ...
        <div className="container mx-auto py-20 pb-16">
            <h1 className="dark:text-white text-blue-gray-800 text-3xl font-bold mt-14 mb-4">{t('Botas y Botines para Hombre')}</h1>
            {zapass.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {zapass.map((zapatoBota) => (
                        <div
                            ...
                            onClick={() => navigate(`/comprar/BotasYBotinesHombre/${zapatoBota.nombre}`)}
                        >
                            <img
                            ...
                            {isAdmin && (
                                <div className="mt-4 flex justify-between">
                                    <Button size="sm" color="blue" onClick={(e) => { e.stopPropagation(); handleOpenEdit("BotasYBotinesHombre", zapatoBota); }}>{t('Editar')}</Button>
                                    <Button size="sm" color="red" onClick={(e) => { e.stopPropagation(); deleteTableData("BotasYBotinesHombre", zapatoBota.id); }}>{t('Borrar')}</Button>
                                </div>
                            )}
                        </div>
                    ))}
